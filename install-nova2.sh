...
case "$variant" in
    ...
    nova2)
        ...
        ;;
    *)
        printf 'Variante inconnue : %s (bar, nova-lite ou nova2 attendu)\n' "$variant" >&2
        exit 2
        ;;
esac
...
if [ "$action" != install ]; then
    printf 'Usage : %s [check|install] [bar|nova-lite|nova2]\n' "$0" >&2
    exit 2
fi
